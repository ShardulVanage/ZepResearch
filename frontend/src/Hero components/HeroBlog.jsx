...
const BlogCard = memo(({ blog }) => (
  <NavLink to={`/Blogs/${blog.id}`}>
    <article className="relative sm:h-[500px] h-auto isolate flex flex-col justify-end overflow-hidden rounded-2xl bg-gray-900 px-8 pb-8 pt-80 sm:pt-48 lg:pt-80">
      ...
      <h3 className="text-lg font-semibold leading-6 text-white text-left">
...
export default function Blogs() {
  ...
  const getRandomBlogs = useCallback((blogs, count) => {
    return [...blogs].sort(() => 0.5 - Math.random()).slice(0, count)
  }, [])
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        ...
        const randomBlogs = getRandomBlogs(formattedBlogs, 3)
